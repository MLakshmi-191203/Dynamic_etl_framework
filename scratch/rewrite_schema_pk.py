...
def get_existing_metadata(conn, config):
    cur = conn.cursor()
    cur.execute('''
        SELECT SOURCE_COLUMN_NAME, SOURCE_DATA_TYPE, SOURCE_ORDINAL_POSITION, IS_PRIMARY_KEY, MTD_TYPE
        FROM dyn_etl.metadata
        WHERE SOURCE_TABLE_NAME = %s
    ''', (config["source_table_name"],))
    return [{"column_name": r[0], "data_type": r[1], "ordinal_position": r[2], "is_primary_key": r[3] == 'Y', "mtd_type": r[4]} for r in cur.fetchall()]
    ...
    for i, col in enumerate(source_schema):
        ...
        if col_name not in tgt:
            ...
            if old_data.get("mtd_type") == "MANUAL":
                # Override source schema with manual definitions so they don't get overwritten
                col["is_primary_key"] = old_data.get("is_primary_key")
                # We could also override data_type but that's more complex.
